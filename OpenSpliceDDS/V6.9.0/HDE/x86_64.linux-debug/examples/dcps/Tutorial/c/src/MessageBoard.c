...
#endif
{
    ...
    if (argc > 1) {
        parameterList->_buffer[0] = DDS_string_dup (argv[1]);
        checkHandle(parameterList->_buffer[0], "DDS_string_dup");
    }
    else
    {
        parameterList->_buffer[0] = DDS_string_alloc(1);
        checkHandle(parameterList->_buffer[0], "DDS_string_alloc");
        strncpy (parameterList->_buffer[0], "0", sizeof(parameterList->_buffer[0]));
    }
