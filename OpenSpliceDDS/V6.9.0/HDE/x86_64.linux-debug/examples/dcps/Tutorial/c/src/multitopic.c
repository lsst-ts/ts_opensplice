    ...
    const DDS_StringSeq *expression_parameters )
{
    ...
    strncpy(nameFinderParams->_buffer[0], "0", sizeof(nameFinderParams->_buffer[0]));
    ...
    for (i = 0; i < msgSeq._length; i++)
    {
        if (infoSeq1._buffer[i].valid_data)
        {
            ...
            if (msgSeq._buffer[i].userID != previous)
            {
                 ...
                 */
                status = DDS_RETCODE_NO_DATA;
                while (status == DDS_RETCODE_NO_DATA && count < 10)
                {
                    ...
                    {
                        userName = DDS_string_dup(nameSeq._buffer[0].name);
                        checkHandle(userName, "DDS_string_dup");
