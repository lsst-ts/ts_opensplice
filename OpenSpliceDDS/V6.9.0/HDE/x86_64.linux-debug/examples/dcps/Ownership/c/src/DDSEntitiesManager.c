...
{
   char* typeName = OwnershipData_StockTypeSupport_get_type_name(typeSupport);

   g_status = OwnershipData_StockTypeSupport_register_type(typeSupport, g_domainParticipant, typeName);
   checkStatus(g_status, "OwnershipData_StockTypeSupport_register_type");
...
DDS_DataWriter createDataWriter(DDS_Publisher publisher, DDS_Topic topic, DDS_boolean isAutodispose, DDS_long strength)
{
   ...
   dataWriterQos->writer_data_lifecycle.autodispose_unregistered_instances = isAutodispose;
   printf("\n=== [Publisher] autodispose_unregistered_instances = %s (%d)", (isAutodispose == TRUE ? "TRUE" : "FALSE"), isAutodispose);

   dataWriterQos->ownership_strength.value = strength;
...
{
   g_status = OwnershipData_StockDataWriter_write(dataWriter, OwnershipDataSample, userHandle);
   checkStatus(g_status, "OwnershipData_StockDataWriter_write");
}
