...
void registerMsgType(DDS_TypeSupport typeSupport)
{
   char* typeName = LifecycleData_MsgTypeSupport_get_type_name(typeSupport);

   g_status = LifecycleData_MsgTypeSupport_register_type(typeSupport, g_domainParticipant, typeName);
   checkStatus(g_status, "LifecycleData_MsgTypeSupport_register_type");
...
DDS_Topic createTopic(const char *topicName, const char *typeName)
{
   ...
   //topicQos->destination_order.kind = DDS_BY_SOURCE_TIMESTAMP_DESTINATIONORDER_QOS;
    ...
    g_status = DDS_DomainParticipant_set_default_topic_qos(g_domainParticipant, topicQos);
    checkStatus(g_status, "DDS_DomainParticipant_set_default_topic_qos");
...
DDS_DataWriter createDataWriter(DDS_Publisher publisher, DDS_Topic topic, DDS_boolean isAutodispose)
{
   ...
   dataWriterQos->writer_data_lifecycle.autodispose_unregistered_instances = (DDS_boolean) ( isAutodispose == TRUE ? 1 : 0 );
